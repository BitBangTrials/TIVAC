...
//Declaring global temperature variables.  To be made internal to function in future iterations.
uint32_t ui32ADC0Value[8];
volatile uint32_t ui32TempAvg;
volatile uint8_t ui8TempValueFSend;
volatile uint8_t ui8TempValueFReceive;
volatile uint8_t ui8TempValueCSend;
volatile uint8_t ui8TempValueCReceive;
...
#else
//Modifying clock frequency for our device.
...
//                   SYSCTL_XTAL_16MHZ);
    ...
    ui8TempValueCSend = (1475 - ((2475 * ui32TempAvg)) / 4096)/10; //Calculate degrees C.
    ui8TempValueFSend = ((ui8TempValueCSend * 9) + 160) / 5; //Calculate degrees F.

    //
    // Initialize the data to send.
    //
    pui32DataTx[0] = ui8TempValueFSend;
    pui32DataTx[1] = ui8TempValueCSend;
    ...
    for(ui32Index = 0; ui32Index < NUM_SSI_DATA; ui32Index++)
    {
        ...
        //
        UARTprintf("'%i' ", pui32DataTx[ui32Index]);
    ...
    for(ui32Index = 0; ui32Index < NUM_SSI_DATA; ui32Index++)
    {
        ...
        //
        // Save data received into the array.
        //
        ui8TempValueFReceive = pui32DataRx[0];
        ui8TempValueCReceive = pui32DataRx[1];
     }
    //Display the temperatures received through the SPI loopback.
    UARTprintf("Current Device Temperature in Fahrenheit: %i\n", ui8TempValueFReceive);
    UARTprintf("Current Device Temperature in Celsius: %i\n", ui8TempValueCReceive);
    }
    //
    // Return no errors
    //
